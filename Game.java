...
//    }
    ...
    class WebcamFeed implements Runnable{
        ...
        Mat topo = Imgcodecs.imread("Resources/images/top.png",1);
...
//            sessao.setTempoTotal(partida.getTempo());
            ...
            while(webSource.grab()){
                try{
                    ...
                    Core.addWeighted(roiTopo,1.0,topo,1.8,0.0,dst);
...
//                    dst.copyTo(cenario.colRange(615,640).rowRange(0,27));
            ...
            while(webSource.grab()){
                try{
                    ...
                    dst = new Mat();
                    Mat roiTopo = cenario.submat(new Rect(new Point(0, 0),new Point(640, 90)));
                    Core.addWeighted(roiTopo,1.0,topo,0.8,0.0,dst);
                    dst.copyTo(cenario.colRange(0,640).rowRange(0,90));
...
//                    }
                    ...
                    putText( roiFase, "Fase: " + partida.getFase().getNumeroFase() + "  de " + qtdFases, new Point(10,20), Core.FONT_HERSHEY_SIMPLEX,0.5,new Scalar(0,0,0),1,8,false );
...
//                    putText( roiFase, partida.getFase().getNome(), new Point(5,20), Core.FONT_HERSHEY_SIMPLEX,0.5,new Scalar(0,0,0),1,8,false );
                    ...
                    putText( roiNivel ,"Nivel: " + String.valueOf(partida.getNivel().getNumero()) + " de " + qtdNiveis, new Point(10,20), Core.FONT_HERSHEY_SIMPLEX,0.5,new Scalar(0,0,0),1,8,false );
...
//                    
                    if (irParaProximaLinha == false){
                        if ((60*minutos + segundos) - (60*minutosAux2 + segundosAux2) < 2){
                            ...
                            if (ultimaLinha){ //pra nao sobrepor imagens de feedback
...
//                        }else{//teste, este bloco de código ficava no if, não no else
                            ...
                            //System.out.println("tempoJogada: " + tempoJogada);
                                ...
                                System.out.println("                                                                  else testeeeeee");
...
//                                                          180                             0                           78                                  78
                           // System.out.println("REF" + "X:"+referencia.getX() + "" + "Y:"+referencia.getY() + "" + "L:"+referencia.getWidth()+ "" + "A:"+referencia.getHeight());
                            ...
                            if(mostrarReferencias && numAcertosNaRodada < partida.getNivel().getQIO()){ //No lugar do '3' seria partida.getNivel().getQIO() ????
                                //variável booleana que é desabilitada quando a função ocultaReferencia é chamada
              ...
              //          if(Calendar.getInstance().getTimeInMillis()>(gerarRodada.getTimeInMillis()+tempoExposicaoReferencia)){
                            ...
                            topo = Imgcodecs.imread("Resources/images/top.png",1);
                            ...
                            float diferenca = ((mostrarBlobs.getTimeInMillis()+tempoExposicao) - Calendar.getInstance().getTimeInMillis());
                            ...
                            Imgproc.line(cenario, new Point(225, 85), new Point(225+(diferenca/te)*200, 85), new Scalar(0, 0, 255, 255),5);
                            ...
                            if(Calendar.getInstance().getTimeInMillis()>mostrarBlobs.getTimeInMillis()+200){
                                ...
                                if(houveColisao==1){
                                    ...
                                    if (tipoColisao == 1){ //acertou
                                        topo = Imgcodecs.imread("Resources/images/topAcerto.png",1);
                                    }else if (tipoColisao == 2){ //errou
                                        topo = Imgcodecs.imread("Resources/images/topErro.png",1);
                                    }
...
//                        }
    ...
    void verificaTransicaoDeNivel(Partida partida) throws UnsupportedAudioFileException, IOException, LineUnavailableException{
           ...
           if((desempenhoMotor<0.3)&&(desempenhoCognitivo>0.6)){
...
//                clip.start(); 
                ...
                Imgproc.resize(avancaLinha, avancaLinha, new Size(350.0, 350.0));
                dst = new Mat();
                Mat roiImgAvancaLinha = cenario.submat(new Rect(new Point(150, 100),new Point(500, 450)));
                Core.addWeighted(roiImgAvancaLinha,0.0,avancaLinha,1.0,0.0,dst);
                dst.copyTo(cenario.colRange(150,500).rowRange(100,450));

                break;
            case 1:
//                File yourFile1 = new File("Resources/sounds/acertou1.wav");
...
//                clip1.start();
                ...
                Imgproc.resize(avancaNivel, avancaNivel, new Size(350.0, 350.0));
                dst = new Mat();
                Mat roiImgAvancaNivel = cenario.submat(new Rect(new Point(150, 100),new Point(500, 450)));
                Core.addWeighted(roiImgAvancaNivel,0.0,avancaNivel,1.0,0.0,dst);
                dst.copyTo(cenario.colRange(150,500).rowRange(100,450));

                break;
            case 2:
//                File yourFile2 = new File("Resources/sounds/acertou1.wav");
//                AudioInputStream stream2;
//                AudioFormat format2;
//                DataLine.Info info2;
//                Clip clip2;
//
//                stream2 = AudioSystem.getAudioInputStream(yourFile2);
...
//                clip2.start();
                ...
                break;
            case 3:
//                File yourFile3 = new File("Resources/sounds/acertou1.wav");
//                AudioInputStream stream3;
//                AudioFormat format3;
//                DataLine.Info info3;
//                Clip clip3;
//
//                stream3 = AudioSystem.getAudioInputStream(yourFile3);
...
//                clip3.start();
                ...
                break;
            case 4:
//                File yourFile4 = new File("Resources/sounds/acertou1.wav");
//                AudioInputStream stream4;
//                AudioFormat format4;
//                DataLine.Info info4;
//                Clip clip4;
//
//                stream4 = AudioSystem.getAudioInputStream(yourFile4);
...
//                clip4.start();
                ...
                break;
            case 5:
//                File yourFile5 = new File("Resources/sounds/acertou1.wav");
//                AudioInputStream stream5;
//                AudioFormat format5;
//                DataLine.Info info5;
//                Clip clip5;
//
//                stream5 = AudioSystem.getAudioInputStream(yourFile5);
...
//                clip5.start();
                ...
                break;
            case 6:
//                File yourFile6 = new File("Resources/sounds/acertou1.wav");
//                AudioInputStream stream6;
//                AudioFormat format6;
//                DataLine.Info info6;
//                Clip clip6;
//
//                stream6 = AudioSystem.getAudioInputStream(yourFile6);
...
//                    dst.copyTo(cenario.colRange(220,420).rowRange(230,480));
    ...
    public void geraRelatorioErros (Mat cenario, Rodada rodada, Partida partida, int numErros) throws AWTException, IOException{
        ...
        /*void gerarImagens(Grade grade, Partida partida, int numSimbolosParaGerar, boolean emitirSom,boolean isReferencia) throws LineUnavailableException, UnsupportedAudioFileException, IOException{
            int numSimbolosGeradosNaRodada = 0;
            System.out.println("\nEntrou no gerar imagens");
            if(numSimbolosFilaGeradosAtual >= partida.getFilaElementos().size()){
                partida.shuffleElements();
                numSimbolosFilaGeradosAtual = 0;
            }
            
            int resultado = partida.getFilaElementos().size()-numSimbolosFilaGeradosAtual;
            int COT_SIZE = partida.getConjuntoImagem().getImagens().size();
            int GRADE_MAX = 6;
            
          //  estava assim no c++
           // #define GRADE_G_MAX 6
           // #define GRADE_M_MAX 16
          //  #define GRADE_P_MAX 78
            ...
            int QIS = partida.getNivel().getQIS();
            
            if((partida.getNivel().getLAD()==1)||(partida.getNivel().getLAD()==2)){
                if(resultado>0){//ainda existem elementos a serem sorteados
                    if(QIS>resultado){// nao tem como gerar todos, "vai faltar elementos na fila" (porque os outros ja foram gerados)
                        if(QIS <= (int)(GRADE_MAX*0.8)){//QSS cabe nos 80% da grade
                            partida.shuffleElements();
                            numSimbolosFilaGeradosAtual=0;
                            numSimbolosParaGerar=QIS;
                        }else{//QSS nao cabe nos 80% da grade, coloca o que couber
                            partida.shuffleElements();
                            numSimbolosFilaGeradosAtual=0;
                            numSimbolosParaGerar=(int)(GRADE_MAX*0.8);
                        }
                    }else{//entao QSS é maior que resultado
                        if(QIS <= (int)(GRADE_MAX*0.8)){//QSS cabe nos 80% da grade
                            numSimbolosParaGerar=QIS;
                        }else{
                            numSimbolosParaGerar=(int)(GRADE_MAX*0.8);
                ...
                }else{//entao resultado ==0
                    // todos os elementos foram sorteados (ou nunca foram), portanto, devemos embaralhar e zerar o cont da fila
                    if(QIS <= (int)(GRADE_MAX*0.8)){//QSS cabe nos 80% da grade
                            partida.shuffleElements();
                            numSimbolosFilaGeradosAtual=0;
                            numSimbolosParaGerar=QIS;
            ...
            }else{//entao serão gerados em ambos os lados
                if(resultado>0){//ainda existem elementos a serem sorteados
                    if((2*QIS)<=COT_SIZE){// signifca que a COT tem elementos suficientes para gerar nos dois lados
                        if( (2*QIS) <= resultado ){ // signifca que a fila TEM elementos suficientes para gerar nos dois lados
                            // entao da para gerar dos dois lados
                            if(QIS <= (int)(GRADE_MAX*0.8)){//QSS cabe nos 80% da grade
                                numSimbolosParaGerar=QIS;
                            }else{
                                numSimbolosParaGerar=(int)(GRADE_MAX*0.8);
                            }
                        }else{// a fila NAO tem elementos suficientes para gerar nos dois lados
                            if(QIS <= (int)(GRADE_MAX*0.8)){//QSS cabe nos 80% da grade
                                partida.shuffleElements();
                                numSimbolosFilaGeradosAtual=0;
                                numSimbolosParaGerar=QIS;
                        ...
                        // COT nao tem elementos suficientes para gerar nos dois lados
                        //portanto, vamos gerar tudo (ou quase tudo (se for impar)) o que tem no COT distribuindo nos dois lados
                        // nao vamos tratar o caso de ser impar, vai faltar apenas um
                        if(QIS <= (int)(GRADE_MAX*0.8)){//QSS cabe nos 80% da grade
                            numSimbolosParaGerar=QIS;
                        }else{
                            numSimbolosParaGerar=(int)(GRADE_MAX*0.8);
            ...
            MTRandom number = new MTRandom();
            for(int i=0;i<numSimbolosParaGerar;i++){
                //o primeiro vai ser a referencia
                if((i==0)&&(isReferencia==true)){
                    ...
                    Imagem imgTemp = new Imagem();
                    
                    imgTemp.setSom(partida.getFilaElementosReferencia().firstElement().getSom());
                    //aqui embaixo era pra ser first element, mas entao teria que modificar a estrutura do arquivo COT
                    //pra incluir um campo GRUPO, pra identificar as imagens de um mesmo contexto
                    imgTemp.setId(partida.getFilaElementosReferencia().lastElement().getId());
                    imgTemp.setImgStr(partida.getFilaElementosReferencia().firstElement().getImgStr());
                    imgTemp.setDescricao(partida.getFilaElementosReferencia().firstElement().getDescricao());
                    
                    Mat temp = new Mat().zeros(80, 80,CvType.CV_32F);
                    
                    temp = partida.getFilaElementosReferencia().firstElement().getImg();
                    Imgproc.resize(temp, temp, new Size(width, height));
                    imgTemp.setImg(temp);
                    grade.getRegioes().elementAt(posicao).setImg(imgTemp);
                    ...
                    //incrementa o num de imagens que estao na grade
                    int numImagens = grade.getNumImagens();
                    grade.setNumImagens(numImagens+1);
                    
                    referencia.setWidth(partida.getNivel().getTAI());
                    referencia.setHeight(partida.getNivel().getTAI());
                    referencia.setX(200);
                    referencia.setY(15);
                    
                    Mat tempRef = new Mat().zeros(80, 80, CvType.CV_32F);
                    
                    
                    //era first ao invés de last, mas a mesma explicação de cima
                    //vai fazer com que pegue um diferente para mostrar na barra de exposição
                    tempRef = partida.getFilaElementosReferencia().lastElement().getImg();
                    Imgproc.resize(tempRef,tempRef,new Size(width, height));
                    referencia.setImagem(tempRef);
                    referencia.setId(partida.getFilaElementosReferencia().lastElement().getId());
                    
                    //coloca as descricoes
                    referencia.setRefImgStr(partida.getFilaElementosReferencia().lastElement().getDescricao());
                    referencia.setSom(partida.getFilaElementosReferencia().lastElement().getSom());
                    
                    numSimbolosFilaGeradosAtual++;
                    numSimbolosGeradosNaRodada++;
                
                }else{
                    if(numSimbolosGeradosNaRodada<partida.getFilaElementos().size()){
                        //gera uma posicao aleatoria
                        int posicao = number.nextInt(grade.getRegioes().size());
                        while(posicoesOcupadas.get(posicao)==1)
                            posicao = number.nextInt(grade.getRegioes().size());
                        ...
                        if(partida.getFilaElementos().firstElement().getId()!=referencia.getId()){
                            Imagem imgTemp = new Imagem();
                            imgTemp.setSom(partida.getFilaElementosReferencia().firstElement().getSom());
                            imgTemp.setId(partida.getFilaElementosReferencia().firstElement().getId());
                            imgTemp.setImgStr(partida.getFilaElementosReferencia().firstElement().getImgStr());
                            imgTemp.setDescricao(partida.getFilaElementosReferencia().firstElement().getDescricao());

                            Mat temp = new Mat().zeros(80, 80,CvType.CV_32F);

                            temp = partida.getFilaElementos().firstElement().getImg();
                            Imgproc.resize(temp, temp, new Size(width, height));
                            imgTemp.setImg(temp);
                            grade.getRegioes().elementAt(posicao).setImg(imgTemp);
                            ...
                            //realmente nao sei o que fazer aqui, manolo fez assim....
                            Imagem hue = new Imagem();
                            hue = partida.getFilaElementos().remove(0);
                            partida.getFilaElementos().add(hue);
                            partida.getFilaElementos().remove(0);
                            
                        }else { // se for igual ao da referencia entao tem que pegar outro (o proximo da fila, se tiver)
                            int resultado1 = partida.getFilaElementos().size()-numSimbolosFilaGeradosAtual;
                            if(resultado1>=2){
                                ...
                                partida.getFilaElementos().add(hue);
                                partida.getFilaElementos().remove(0);
                            }else{// se tem apenas um sobrando (o que sobrou eh igual ao da referencia)
                                //nao sorteia nenhum
                            }
                        }
                    }
                }
            }//sim for numero simbolos para gerar
            ...
            //tempoExposicao = partida.getNivel().getTES();
